...
2026-09-16 用户已恢复原始 JSON，全量规则审计复现 970 源 / 131 受阻 / 86.5% dummy 执行无错误，与历史报告无差异；这不是在线可用率。输入哈希及命令见 [E0 重测记录](docs/e0-rule-authority-2026-09-16.md)。
...
- [x] **受阻源 131 → 门槛见 §6.2**（2026-09-16 恢复语料并全量重测，131 → 131，满足 ≤140；≤90 尚未达到）。每次改动重跑 `rule-audit`，把受阻源变化写进 commit message
...
| --- | ---: | ---: |
| 当前（v3 写时） | 182 | −18.8% |
| **当前重测（2026-09-16，与历史一致）** | **131** | **−41.5%** |
| **v0.8.0 门槛** | **≤ 140** | −37.5% |
| **v1.0.0 门槛** | **≤ 90** | −60% |
...
| **v0.9.0** | **F3 + E0** | 阅读器七区全建 + 精确分页 + 读完一本在线书 · 受阻源 ≤ 140 + `selector.rs` 兜底已删 | 🟡 F3 实现就位（运行时未验）；E0 全量重测受阻源 **131（2026-09-16）**，`selector.rs` 兜底未删 |
...
4. E0：语料已恢复且全量重测仍为 131 受阻；继续按真实 fixture 修 JS/规则语义，删 selector 四处兜底并完成存储迁移。静态审计不替代管线提取正确性与在线验收。
