...
| 维度 | v3 记录 | 实测 | 说明 |
| --- | --- | --- | --- |
| 受阻源 | 182 | **165** | 相对 224 基线 −26.3%；v0.8.0 门槛 ≤140 仍差 25 源 |
| 无错误源 | 788（81.2%） | **805（83.0%）** | 规则串 22,220 未变 |
| 受阻源大类 | js runtime 123 源 · css 77 源 | **js runtime 106（单独修可解 64）· css compatibility 77（可解 41）· path parser 16（可解 11）· JVM 14（永久不可解）· harness 4** | js runtime 仍是主战场 |
...
| **v0.9.0** | **F3 + E0** | 阅读器七区全建 + 精确分页 + 读完一本在线书 · 受阻源 ≤ 140 + `selector.rs` 兜底已删 | 🟡 F3 实现就位（运行时未验）；E0 1/8，门禁已修，受阻源 165 |
...
| 3 | E0 · **模板串被当 CSS 选择器**：`{$.grade}分`（12 条）· `{$.type_name},{$.catalog_name}`（9 条）· `连载中{$.status}已完结` 一族 | 独立，可与 #2 并行；css compatibility 77 源、可解 41 源 | E0 / §6.2 |
