...
**一句话现状：后端 50 个 IPC 命令、148 个 Rust 文件、静态覆盖率 86.3%（受阻 133 源），已经能撑起原型里
...
| 维度 | v3 记录 | 实测 | 说明 |
| --- | --- | --- | --- |
| 受阻源 | 182 | **133** | v3 写后复核为 165；两批引擎修复后 **133**。相对 224 基线 −40.6%，**已过 v0.8.0 门槛（≤140）** |
| 无错误源 | 788（81.2%） | **837（86.3%）** | 规则串 22,220 未变 |
| 受阻源大类 | js runtime 123 源 · css 77 源 | **js runtime 105（单独修可解 81）· css compatibility 23（可解 17）· unsupported JVM 14（永久不可解）· unimplemented hook 12（可解 7）· harness 4 · path parser 2** | css 与 path parser 两桶基本清空；js runtime 仍需分辨真缺口与假载荷（见 `docs/coverage/failure-worklist.md`） |
...
| --- | ---: | ---: |
| 当前（v3 写时） | 182 | −18.8% |
| **当前实测（2026-09-10，两批引擎修复后）** | **133** | **−40.6%** |
| **v0.8.0 门槛** | **≤ 140** | −37.5% |
| **v1.0.0 门槛** | **≤ 90** | −60% |
...
| **v0.9.0** | **F3 + E0** | 阅读器七区全建 + 精确分页 + 读完一本在线书 · 受阻源 ≤ 140 + `selector.rs` 兜底已删 | 🟡 F3 实现就位（运行时未验）；E0 受阻源 **133 已达标**，`selector.rs` 兜底未删 |
...
| 3 | E0 · **规则方言 + URL 字段批次**：显示模板 `{$.x}` 路由、JSoup `[attr~=regex]`、前导 `+`、属性运算符空格、审计区分配置字段与钩子；URL 字段改走 `AnalyzeUrl` 语义（相对 URL 渲染而非当 XPath） | ✅ 已完成：受阻源 165 → **133**（过 v0.8.0 门槛），css compatibility 77 → 23 源、path parser 16 → 2 源 | §0.4 / E0 |
