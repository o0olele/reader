...
>
> **最近复核：2026-09-02。** §0 是 2026-08-30 的**起点快照，已成历史记录**，保留用于对照；
> **当前真实进度一律以 §10 为准。**§10 的每一条都要求「代码里能验证到什么」，不接受未经核对的打勾。

**一句话现状：Step 0 ✅ · Step 1 代码项 ✅（待真实书源手工验收）· Step 2 进行中（规则引擎、基础 JS Runtime 与协议认证状态闭环已接入，剩 WebJS/完整网络兼容层、调试器与公开语料验证）。**
...
**2026-09-02 进展：** 协议认证的第一段闭环已落地。普通搜索/书源探针收到 HTTP 401/403
会把会话持久化标记为过期（保留 token/cookie 供用户刷新），探针返回请求 URL、HTTP 状态、
认证要求和会话状态；QuickJS 规则可通过 `java.sessionState()` / `java.isAuthenticated()`
读取同一状态，且 `java.post` / `java.ajax` 支持省略 body 或 method。新增仓库、服务和运行时回归测试，
避免失效凭据继续被静默使用。仍待：完整 `java.*` 请求选项/响应头 API、调试器 UI 展示和 WebView Cookie 回写。
