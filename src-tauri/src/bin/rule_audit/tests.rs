    ...
    fn mixed_sources_use_html_for_html_stages_and_json_for_json_rules() {
        assert_eq!(dummy::input_for("[property=og:title]@content", true), dummy::HTML);
    ...
    fn classifies_nested_json_path_errors() {
        assert_eq!(error_category("source error: json path is invalid: unexpected character at 14"), "path parser");
    }
