...
pub(super) fn markdown(report: &Audit) -> String {
    ...
    out.push_str("Input types are inferred from source metadata. Empty matches count as successful execution; this report does not establish extraction correctness or online availability. JSON field paths and display templates are executed by the production engine. Failure locations use zero-based corpus array indexes.\n\n");
