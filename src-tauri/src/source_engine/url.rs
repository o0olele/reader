...
pub fn build(
    ...
    let (target, options_text) = if let Some(script) = trimmed
        .strip_prefix("<js>")
    ...
    let method = options
        ...
        .parse()
        .map_err(|e| AppError::InvalidArgument(format!("{label} HTTP method 无效: {e}")))?;
    let body = options
        .get("body")
        .and_then(value_string)
        .map(|value| expand(value.as_str(), keyword.unwrap_or("")));
    let charset = options
        .get("charset")
        .and_then(value_string)
    ...
    Ok(RequestSpec {
        url: resolve_url(&source.base_url, target.trim(), label)?,
        method,
        body,
        charset,
        headers,
    ...
    for (index, ch) in value.char_indices() {
        match ch {
            ...
            ',' if quote.is_none() && depth == 0 => {
                return Ok((&value[..index], Some(value[index + 1..].trim())))
            }
            _ => {}
        }
    }
    if value.starts_with("<js>") {
        return Err(AppError::Parse(format!("{label} JS 缺少请求选项")));
    }
    Ok((value, None))
}
...
fn parse_headers(value: &serde_json::Value) -> Result<Vec<(String, String)>, AppError> {
    ...
    Ok(object
        .iter()
        .map(|(k, v)| (k.clone(), value_string(v).unwrap_or_else(|| v.to_string())))
        .collect())
}
...
mod tests {
    ...
    #[test]
    fn parses_options_and_expands_body() {
        let spec = build(
            &source(),
            "path,{'method':'POST','body':'q={{key}}','retry':2,'headers':{'X-A':'b'}}",
            Some("%E4%B8%AD"),
            "test",
        )
        .unwrap();
        assert_eq!(spec.method, reqwest::Method::POST);
        assert_eq!(spec.body.as_deref(), Some("q=%E4%B8%AD"));
        assert_eq!(spec.retry, 2);
        assert_eq!(spec.headers[0], ("X-A".into(), "b".into()));
    }
    #[test]
    fn parses_js_wrapper() {
        let spec = build(
            &source(),
            "<js>/api,{'charset':'gbk'};result;</js>",
            None,
            "test",
        )
        .unwrap();
