...
use crate::{
    ...
    error::AppError,
    source_engine::{
        legado_rules::{checked_stage, LegadoRules, LegadoTocRule},
...
fn engine_catalog(
    ...
    let Some(list) = rules.chapter_list.as_deref() else {
        return Err(AppError::parse("ruleToc 缺少 chapterList"));
    };
...
pub fn parse_catalog_page(source: &BookSource, html: &str) -> Result<CatalogPage, AppError> {
    let rules = checked_stage(
        source.raw_rules.toc.as_ref(),
        LegadoRules::decode(&source.raw_rules).toc,
        "ruleToc",
    )?;
    if let Some(rules) = rules.as_ref() {
        let chapters = engine_catalog(source, rules, html)?;
