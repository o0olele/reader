...
**一句话现状：架构已达标、规则引擎骨架已通、认证链路已闭环 —— 静态规则覆盖率为 36.6%，最新线上抽样为 249 / 530（47.0%），但失败样本仍以网络不可达/超时为主，阅读器本体仍在可用化过程中。**
...
```
上一轮返回结果：158 / 530 = 29.8%
上一轮失败：    372 / 530 = 70.2%

最新返回结果：249 / 530 = 47.0%
最新失败：    281 / 530 = 53.0%
相对上一轮：  +91 个源，+17.2 个百分点
```
...
### 0.4.1 最新失败报告（`r.txt`，281 条）

`r.txt` 是用户在同一批 530 源上的最新失败清单。按错误文本逐条归类（类别有重叠，HTTP/JS 错误会同时带有 `source error`）：

| 类别 | 条数 | 说明 |
| --- | ---: | --- |
| 连接超时 | 181 | 主要是源站响应慢、网络/代理不可达，不能归为认证失败 |
| Cloudflare JavaScript challenge | 22 | 需要在认证 WebView 中完成验证；HTTP 客户端不绕过 challenge |
| 无法连接目标站点 | 13 | DNS、防火墙或代理问题 |
| HTTP 403 | 9 | 其中 6 条被标注“需要重新认证”；其余是站点/WAF/JS 请求拒绝 |
| HTTP 404 | 9 | URL 或站点路径已失效 |
| HTTP 400 | 5 | 请求格式、参数或站点端点已变更 |
| HTTP 451 | 5 | 站点法律/地区限制，不应提示重新认证 |
| 规则脚本 `source error` | 24 | QuickJS 兼容缺口或脚本本身异常 |
| 请求 `parse error` | 9 | 书源请求选项/编码等格式问题 |

因此，“大部分需要认证”与报告文本不符：明确带“需要重新认证”的只有 **6 / 281（2.1%）**；可由认证链路直接处理的范围还包括 403 的浏览器重试和 22 条 Cloudflare challenge。最新失败的主因是 **194 条连接类失败（181 超时 + 13 无法连接，占 69.0%）**。本轮已补上：已有认证 WebView 时，401/403 会先走一次浏览器请求；JS 规则产生的普通 401/403 也会被归类为认证失败，而 Cloudflare interstitial 仍不会误标为会话过期。

最新成功率 **47.0%** 相比上一轮 **29.8%** 已提升 **17.2 个百分点**，但仍不能作为 v0.3.0 的通过标准。必须先落地 `source-audit`，把网络、认证、Cloudflare、脚本、解析为空和站点失效拆成可复现的阶段统计。
...
> 2026-09-05 完成 JSONPath 数组展开、QuickJS `androidId` 兼容和 CSS fallback 修复后，严格口径升至 **355 / 970 = 36.6%**；线上抽样随后从 **158 / 530（29.8%）** 提升到 **249 / 530（47.0%）**，两者仍须分开报告。
...
`getReadBookConfig*` / `getThemeConfig*`；`queryTTF` / `replaceFont`（字体反爬）。
...
| **线上可用率仅 47.0%** | 端到端搜索仍有 53.0% 源失败 | **更新（2026-09-05）** —— 最新 530 源抽样得到 249 个结果（上一轮 158 个）；`r.txt` 显示失败主因是连接超时/不可达（194 条），明确“需要重新认证”仅 6 条，另有 Cloudflare challenge、脚本和站点失效，必须由待实现的 `source-audit` 分层统计，不能用静态覆盖率替代 |
...
| 3 | **`source-audit` 在线 harness**：记录 URL / HTTP / 阶段 / 失败分类并落 CSV | 线上 530 源结果可复现 | §3.3 | 🟡（`src-tauri/src/bin/source-audit.rs` 已落地；待真实网络批次验收） |
