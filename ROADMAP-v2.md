...
- [ ] 745 条 harness 失败降到 100 条以内（待完整语料重跑确认）
- [ ] 修完立刻重跑，得到**第一个可信的静态覆盖率**（待完整语料重跑确认）
...
状态：🟡 已完成上下文别名与宽松求值对齐；运行时按职责拆分仍待完成。
...
| 6 | **拆 `js_runtime.rs`（2,080 行）并对齐 JS 语义** | 上下文/宽松求值已完成；文件拆分待完成 | §3.4 |
