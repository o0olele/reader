...
- [ ] 745 条 harness 失败降到 100 条以内
- [ ] 修完立刻重跑，得到**第一个可信的静态覆盖率**（预期显著高于 36.6%）
...
- [ ] 报告能直接回答：「N 个受阻源中，TOP5 阻塞类别各卡住多少个源」
...
- 注入 legado 的完整上下文变量集（`src` `title` `result` `baseUrl` `book` `chapter` `source` `cookie`）
- 以非严格模式求值以支持 `with` 与重复声明
- 按 `not a function` 的实际清单补剩余 `java.*`
- **`js_runtime.rs` 2,080 行必须在本节拆开**：`runtime.rs` + `bindings/{net,codec,rule,ctx,crypto}.rs`

⛔ 不做：`toast` / `longToast`（给空实现）· `getReadBookConfig*` · `queryTTF` / `replaceFont`

### 3.5 模板容错

`engine.rs:167` 的 `template is missing }}` 改为：未闭合时按字面量输出剩余文本。
...
- [ ] 静态覆盖率：以 P0 修正后的口径为基线，**受阻源数下降 ≥ 70%**
...
| **v0.2.0** | **P0** | rule-audit harness 修正 + 按源归因表 + `txtTocRule` 移植 | 🟡 |
...
| **Java 正则与 Rust `regex` 差异** | 263 条规则失败，第二大真实缺口 | **v1 登记、三版未关闭，现已量化** —— P1 §3.2 关闭 |
| **JSoup 选择器宽松度未对齐** | 202 条规则失败 | **本次新增** —— 照字段表 / token 表都看不见，只有跑真实语料才暴露 |
...
| # | 任务 | 产出 | 节 |
| ---: | --- | --- | --- |
| 1 | **修 `rule-audit.rs` 的假输入**：按书源判定 JSON/HTML，而非按规则前缀 | 745 条假失败清零，得到第一个可信覆盖率 | §2.1 |
| 2 | **报告按源归因**：`blocked_by: Category → Set<source_id>` | 能回答「哪类问题卡住多少个源」 | §2.2 |
| 3 | **严格度对齐**：空规则 / 空分支 / 未闭合引号降级为空结果 | 338 条失败，最便宜的一项 | §3.1 |
| 4 | **Java 正则归一化层** | 263 条，关闭 v1 就登记的老风险 | §3.2 |
| 5 | **JSoup 选择器归一化 + `:contains()` / `:eq()`** | 202 条 | §3.3 |
| 6 | **拆 `js_runtime.rs`（2,080 行）并对齐 JS 语义** | 160 条 + 关闭纪律违规 | §3.4 |
